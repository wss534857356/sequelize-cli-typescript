...
function ensureMigrationsFolder() {
  if (!_helpers2.default.path.existsSync(_helpers2.default.path.getMigrationsSourcePath())) {
    _helpers2.default.view.error('Unable to find migrations (source) path (' + _helpers2.default.path.getMigrationsSourcePath() + '). Did you run ' + _cliColor2.default.blueBright('sequelize init') + '?');
  }

  if (!_helpers2.default.path.existsSync(_helpers2.default.path.getMigrationsCompiledPath())) {
    _helpers2.default.view.error('Unable to find migrations (compiled) path (' + _helpers2.default.path.getMigrationsCompiledPath() + '). Did you compile your migrations?');
  }
}
